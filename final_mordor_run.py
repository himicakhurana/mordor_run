...
background_image2 = pygame.image.load('background2.jpg').convert()
background_image3 = pygame.image.load('background3.jpg').convert()


x, y = 75, 700
font = pygame.font.Font('freesansbold.ttf', 32)
score_text = font.render('Score : 0', True, GREEN, BLUE)
level_text = font.render('Level : 0', True, GREEN, BLUE)
health_text = font.render('Health : 100%', True, GREEN, BLUE)
score_text_rect = score_text.get_rect()
score_text_rect.center = (100, 100)
level_text_rect = level_text.get_rect()
level_text_rect.center = (400, 100)
...
test_group2 = pygame.sprite.Group(obstacle2_sprite)
...
sprite_to_move = obstacle2_sprite
random_sprites = [obstacle2_sprite, score_sprite, health_sprite]
sprite_to_show = 0
for i in range(len(random_sprites)):
    if i != sprite_to_show:
        random_sprites[i].rect.centery = window_height + 10000
        random_sprites[i].rect.centerx = window_width + 400
    else:
        random_sprites[i].rect.centerx = window_width - 100
        random_sprites[i].rect.centery = window_height - 100 - 75
        sprite_to_move = random_sprites[i]
level = 1
...
while not dead:
    ...
    if 3 < score <= 7:
        level = 2
        #print("level 1")
        screen.blit(pygame.transform.scale(background_image2, (window_width, window_height)), (0, 0))
    else:
        if score > 7:
            screen.blit(pygame.transform.scale(background_image3, (window_width, window_height)), (0, 0))
            level = 3
        else:
    ...
    level_message = 'Level : ' + str(level)
    level_text = font.render(level_message, True, GREEN, BLUE)
    ...
    x_position2 = sprite_to_move.rect.centerx - speed2

    if x_position2 > 0:
        sprite_to_move.rect.centerx = x_position2
        sprite_to_move.rect.centery = window_height - 175
    else:
        if sprite_to_show == 2:
            sprite_to_show = 0
        else:
        ...
        for i in range(len(random_sprites)):
            if i != sprite_to_show:
                ...
                sprite_to_move = random_sprites[i]
        sprite_to_move.rect.centerx = window_width
        sprite_to_move.rect.centery = window_height - 175
    ...
    for s in collide:
        ...
        obstacle1_sprite.rect.centerx = window_width
        obstacle1_sprite.rect.centery = window_height - 100
    collide2 = pygame.sprite.spritecollide(character_sprite, test_group2, False)
    for s in collide2:
        health = health - 20
        #print('collision2')
        if sprite_to_show == 2:
...
while not dead:
    for event in pygame.event.get():
        ...
        score_message = 'Game over!  ' \
                        'Score : ' + str(score) + level_message
        score_text = font.render(score_message, True, GREEN, BLUE)
        screen.blit(score_text, score_text_rect)
        # level_text = font.render(level_message, True, GREEN, BLUE)
        # screen.blit(level_text, level_text_rect)

        health_message = 'Health : ' + str(0) + '%'
        health_text = font.render(health_message, True, GREEN, BLUE)
        screen.blit(health_text, health_text_rect)
        pygame.display.flip()
        clock.tick(clock_tick_rate)
